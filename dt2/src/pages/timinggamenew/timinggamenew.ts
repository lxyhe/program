...
import { TimingGameView } from '../timinggameview/timinggameview';
...
@Component({
  selector: 'timinggamenew',
  templateUrl: 'timinggamenew.html'
})
export class TimingGameNew {
  ...
  create() {
    console.log("创建开始")
    if (null == this.title || null == this.addr || null == this.phone || null == this.pcode || null == this.price) {
    ...
    let x = this.ctimggame.createPreviewData(this.title, this.addr, this.phone, this.price, this.pcode, this.titlepic, this.place, this.lng, this.lat, this.zoom,
      this.details_x_1,
      this.details_x_2,
      this.details_x_3,
      this.details_x_4,
      ...
      console.log("数据完成");
      console.log(this.title, this.addr, this.phone,this.price, this.pcode);
    this.nav.push(TimingGameView, x);
